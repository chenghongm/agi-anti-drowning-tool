...
5. Short Mode（短对话压缩）
一键短模式: 提供 Short Toggle，可将当前对话消息压缩为更短的预览视图，减少长对话占屏。

平台适配: 已针对 Grok 平台补充样式适配，确保 Short Mode 下 SER 控件可见，并隐藏原生 action-buttons 以保持界面整洁。

6. Assistant 输出可见性切换
一键切换: 提供 Assistant Toggle，可快速隐藏或显示 AI assistant 输出，帮助用户更清晰地提取和整理自己的思路。
...
Grok Side Controls: `getSideControlTopOffset()` 仅用于 Grok 平台右侧控制组（Short Mode / Assistant / Code / Nav）的顶部定位，目的是避开 Grok 自带 timeline 组件；如需整体下移，只需调整 `baseTop`。
...
5. Short Mode
One-click short mode: Adds a Short Toggle that compresses conversation messages into a smaller preview state, reducing visual load in long threads.

Platform adaptation: Includes Grok-specific short-mode styling so SER controls remain visible while the native action-buttons row is hidden for a cleaner compact view.

6. Toggle Assistant output visibility
...
Grok Side Controls: `getSideControlTopOffset()` is only used for Grok-side control placement (Short Mode / Assistant / Code / Nav). It offsets the whole stack away from Grok's native timeline; if you want to move the Grok controls down, adjust `baseTop`.
...
2. Deployment: Load the script on chatgpt.com or claude.ai or Gemini, or Grok.ai (currently tested on web clients).
...
Roadmap
-------
[ ] Multi-Model Support: Has Completed 100% selector mapping for ChatGPT, Claude, Gemini, and Grok.
